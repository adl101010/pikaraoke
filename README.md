...
- **Multiple TVs** — Point as many screens at the player page as you like; they stay in step with each other. One screen leads: it sets the position the rest follow, and it's the only one allowed to end a song, so a second screen can't cut someone off. Pick which one from the admin panel and keep the sound on that TV, turning it down on the others. Worth pinning even with a single TV — a pinned screen takes the lead straight back after a dropped connection instead of waiting to be noticed.
...
- **yt-dlp release channel** — Stable yt-dlp can trail YouTube's changes by weeks, and when YouTube breaks a player client, downloads start failing with 403s. Pick **Nightly** in the admin panel (or `--ytdl-update-channel nightly`) and hit Update — no container restart needed, and the choice sticks across restarts. The panel tags which build is actually installed, and says so when it doesn't match the channel you picked yet.
- **Downloads retry themselves** — A download refused with a 403, a timeout or a dropped connection is tried again with a widening delay instead of quietly failing. Videos that are genuinely private or missing still fail straight away, rather than making a guest wait to be told no.
